...
public class FilterResourceHelper {
    ...
    public static void generateFilterThumbs(Context context, boolean z) {
        ...
        for (FilterType filterType : FilterType.values()) {
            ...
            BitmapUtils.saveBitmapWithFilterApplied(context, filterType, loadBitmapFromAssets, file2.getAbsolutePath(), null);
            i++;
        }
    ...
    public static String getSimpleName(FilterType filterType) {
        String replaceAll = filterType.name().toLowerCase().replaceAll(TextureMediaEncoder.FILTER_EVENT, "");
